...
$("#item_search").autocomplete({
    source: function(data, cb){
        $.ajax({
        	autoFocus:true,
            url: $("#base_url").val()+'stock/get_json_items_details',
            method: 'GET',
            dataType: 'json',
            data: {
                name: data.term,
...
function return_row_with_data(item_id,stock){
  ...
  var warehouse_id=$("#warehouse_from").val();
	$.post(base_url+"stock/return_row_with_data/"+rowcount+"/"+item_id+"/"+stock,{warehouse_id:warehouse_id},function(result){
        //alert(result);
