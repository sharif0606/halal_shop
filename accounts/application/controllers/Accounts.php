...
class Accounts extends CI_Controller {
	...
	public function sub1_head_list(){
		...
		if(MyAuth::getPageAccess("s1Head")){
			redirect('dashboard');
			exit;
		}
	...
	public function sub2_head_list(){
		...
		if(MyAuth::getPageAccess("s2Head")){
			redirect('dashboard');
			exit;
		}
	...
	public function journal_voucher_entry(){
		$data['page']='accounts/journal_voucher_entry.php';
		$this->load->view('template',$data);
	}
	...
	public function journal_voucher_list(){
			...
			if ($this->form_validation->run() == true){
				...
				if(!empty($voucher_no)){
					$data1 = array(
							...
							'bank'    		=> $bank,
							'rec_date'    	=> $rec_date,
							'createdBy' 	=> $this->session->userdata('admin_logged_in')['id']
							);
		...
		$data['page']='accounts/journal_voucher_list.php';
		$this->load->view('template',$data);
	}
	...
	public function journal_voucher_edit($id){
		...
		$data['page']='accounts/journal_voucher_edit.php';
		$this->load->view('template',$data);
	}
	
	public function journal_voucher_delete($id){
		
		if(MyAuth::getPageAccess("vDelete")){
			redirect('dashboard');
			exit;
		}
	...
	public function journal_voucher_approve($id,$status){
		...
		if(MyAuth::getPageAccess("accApprove")){
			redirect('dashboard');
			exit;
		}
	...
	public function journal_voucher_print($id){
		...
		$data['vdata'] = $this->Accounts_model->journal_voucher_row_edit($my_id,$id);
		$data['bkdndata'] = $this->Accounts_model->jourvoucher_bkdn_row_edit($my_id,$id);
		$html = $this->load->view('accounts/journal_voucher_print', $data, true);
		//$html .= ob_get_clean();
	...
	public function credit_voucher_entry(){
		...
		$data['page']='accounts/credit_voucher_entry';
		$this->load->view('template',$data);
	}		
	...
	public function credit_voucher_list(){
		...
		$data['page']='accounts/credit_voucher_list.php';
		$this->load->view('template',$data);
	}
	...
	public function credit_voucher_edit($id){
		...
		$data['page']='accounts/credit_voucher_edit';
		$this->load->view('template',$data);
	}
